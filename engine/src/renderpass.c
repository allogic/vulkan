...
VkRenderPass g_renderpass_pre_depth = 0;
VkRenderPass g_renderpass_main = 0;

void renderpass_create_pre_depth(void) {
  ...
  VK_CHECK(vkCreateRenderPass(g_window.device, &render_pass_create_info, 0, &g_renderpass_pre_depth));
}
...
void renderpass_destroy_pre_depth(void) {
  vkDestroyRenderPass(g_window.device, g_renderpass_pre_depth, 0);
}
